...
export class GroupCrudService {
  ...
  async getAll(
    ...
    try {
      ...
      return err(new InternalServerErrorException(`Could not get Group Resources.`));
    }
  }
  ...
  async create(data: Prisma.GroupCreateInput): Promise<Result<Group, Error>> {
    try {
      ...
      return err(new InternalServerErrorException(`Could not create Group Resource.`));
    }
  }
  ...
  async delete(id: string): Promise<Result<Group, Error>> {
    try {
      const result = await this.prismaService.group.delete({ where: { id: id } });
      return ok(result);
    } catch (e) {
      return err(new InternalServerErrorException(
